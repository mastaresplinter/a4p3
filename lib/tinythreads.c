...
/** @brief Sorts an element into the queue
 * 
 */
static void enqueue(thread p, thread *queue) {
	...
	else if(p->Period_Deadline < (*queue)->Period_Deadline)
	{
		p->next = *queue;
		*queue = p;
	}
...
void spawnWithDeadline(void (* function)(int), int arg, unsigned int deadline, unsigned int rel_deadline) {
	...
	newp->arg = arg;
	newp->next = NULL;
	newp->Period_Deadline = deadline;				//Set deadline
	newp->Rel_Period_Deadline = rel_deadline;		//Set relative deadline
	if (setjmp(newp->context) == 1) {
		ENABLE();
		current->function(current->arg);
		DISABLE();
		enqueue(current, &doneQ);				//Enqueue doneQ if the current is done.
		current = NULL;
		dispatch(dequeue(&readyQ));
	}
	SETSTACK(&newp->context, &newp->stack);
...
void respawn_periodic_tasks(void) {
	thread p;
	while (doneQ->Period_Deadline == ticks)
	{
		p = dequeue(&doneQ);
		enqueue(p, &freeQ);
		spawnWithDeadline(p->function, p->arg, (p->Rel_Period_Deadline + ticks), p->Rel_Period_Deadline);
	}
}
...
{
	DISABLE();
	if (readyQ != NULL && (readyQ->Period_Deadline < current->Period_Deadline))
	{	
		thread p = dequeue(&readyQ);
		enqueue(current, &readyQ);
		dispatch(p);
...
void scheduler(void)
{
	respawn_periodic_tasks();	//Put the tasks with expired deadlines into the readyQ
	scheduler_EDF();				//Rate monotonic scheduler
}
